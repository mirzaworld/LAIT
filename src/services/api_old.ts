...
// Connects to backend at port 5003 with proper error handling
...
export const invoices = {
   ...
   */
  list: async (page = 1, perPage = 20): Promise<{
    invoices: any[];
    pagination: {
      page: number;
      per_page: number;
    ...
    return await apiRequest(`/api/invoices?page=${page}&per_page=${perPage}`);
  },

  /**
   * Get specific invoice details
   */
  get: async (invoiceId: number): Promise<any> => {
...
export const analytics = {
  ...
  vendors: async (): Promise<{
    vendor_analytics: any[];
    summary: {
      total_vendors: number;
      total_relationships: number;
      avg_vendor_spend: number;
    };
  }> => {
  ...
  /**
   * Get system statistics (if admin)
   */
  systemStats: async (): Promise<{
    total_users: number;
    total_invoices: number;
    ...
    return await apiRequest('/api/system/stats');
  },
};
...
/**
 * Upload invoice file (legacy function for compatibility)
 */
...
/**
 * Login function (legacy for compatibility)
 */
...
/**
 * Register function (legacy for compatibility)
 */
